...
const executeCommanTool = tool(
    async ({ command, workingDirectory }) => {
        ...
        return new Promise((resolve, reject) => {
            // 直接把完整命令交给 shell 执行，避免按空格拆分导致重定向/复合命令失效。
            ...
            child.on('close', (code) => {
                if (code === 0) {
                    ...
                    \n\n重要提示：命令目录"${workingDirectory}"中执行成功。
                    如果需要再这个项目目录中继续执行命令，请使用 workingDirectory
                    ...
                    reject(new Error(`命令执行失败：${command} (exit code: ${code || 1})`));
                }
            })
...
const listDirectoryTool = tool(
    async ({ directoryPath }) => {
        try {
            ...
            console.log(`[工具调用] list_directory("${directoryPath}") 成功读取 ${files.length} 个文件`);
            // 返回目录内容（文件列表）
            return `目录内容: \n ${files.map(f => `- ${f}`).join('\n')}`;
        } catch (error) {
